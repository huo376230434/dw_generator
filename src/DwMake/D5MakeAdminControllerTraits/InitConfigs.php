...
//        模板文件根目录
        $this->stub_dir = $this->getBaseStubDir().'/adminControllerStubs/';

      $this->config_path =  $config_path = resource_path("quickdev/admin_controller_config.php");

        $this->default_config_path = resource_path("quickdev/admin_controller_config_default.php");
        $des_config_dir = resource_path('quickdev/admin_controller_config');
...
//        模型的命名空间
        $this->base_model_namespace = "App\\Model";
...
//        路由的简称，如：/admin
        ...
        $base_name = $this->base_name = ucfirst(camel_case($configs['controller_name']));
//        路由中的控制器名称
...
//        dd($this->filter);
    ...
    protected function getMethod($method,$key=null)
    {
        ...
        if ($method === null && str_contains($key,".")) {
        ...
        return "\App\Model\\".$model."::".$method."Options()";
    }
