...
class DwMakeModels extends Command
{
    use DwMakeTrait;
    /**
        ...
        if (\Schema::hasTable($table_name)) {
            ...
            foreach (\Schema::getColumnListing($table_name) as $item) {
                if (str_is("*_id", $item)) {
                    ...
                    $id_columns[] = $item;
                    $relation_model = ucfirst(camel_case(rtrim($item, "_id")));
...
//                dump($combination);
    ...
    protected function addBelongsToManyRelation($item)
    {
        ...
        $firstModel = ucfirst(camel_case(trim($item[0],"_id")));
        $secondModel = ucfirst(camel_case(trim($item[1],"_id")));
//        dump($firstModel);
//        dump($secondModel);

        $pivot_name = str_plural(snake_case($firstModel.$secondModel));
//        dump($pivot_name);
    ...
    {
        $relation_trait_path = $this->model_dir . $model.".php";
        ...
        if(in_array($relation_type,['hasMany',"belongsToMany"])){
            $method = str_plural($method);//如果是一对多，多对多，则用复数
        }
...
DDD;
    ...
    protected function init_configs()
    {
        ...
        $this->model_traits_dir = $this->model_dir.'Traits/';
        if (!is_dir($this->model_traits_dir)) {
            mkdir($this->model_traits_dir);
        }
